...
async function appendToJournal(entry: JournalEntry) {
  ...
  try {
    ...
    console.log('🔐 PUT headers for journal R2:', putHeaders);
...
async function loadJournal(): Promise<JournalEntry[]> {
  ...
  try {
    ...
    console.log('🔍 Attempting to load journal from R2:', r2Endpoint);
    ...
    if (!response.ok) {
      if (response.status === 404) {
        console.log('📝 No journal found, returning empty array');
        return [];
      }
...
serve(async (req) => {
  ...
  try {
    ...
    if (req.method === 'POST' && action === 'append') {
      ...
      console.log('📝 Appending journal entry:', entry.id, 'boss input:', entry.bossInput.substring(0, 50));
